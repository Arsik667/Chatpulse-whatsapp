...
Язык текстов ошибок задаётся ?lang=ru|en. Сам отчёт от языка не зависит:
предупреждения в нём — коды, их переводит фронтенд.
...
from analyzer_core import DEFAULT_GAP_HOURS, DEFAULT_LANG, ChatParseError, analyze_file, message
...
def _too_large(lang: str) -> HTTPException:
    return HTTPException(413, message("upload_too_large", lang, limit=MAX_UPLOAD_MB))


async def _read_upload(request: Request, lang: str) -> tuple[bytes, str]:
    """Достаёт из multipart-запроса поле file целиком в память."""
    if not request.headers.get("content-type", "").startswith("multipart/form-data"):
        raise HTTPException(415, message("not_multipart", lang))
    ...
    try:
        ...
        raise HTTPException(400, message("bad_form", lang, reason=exc.message)) from exc

    try:
        upload = form.get("file")
        if not isinstance(upload, UploadFile):
            raise HTTPException(400, message("no_file_field", lang))
...
async def analyze(
    ...
    lang: Lang = Query(DEFAULT_LANG, description="Язык текстов ошибок"),
) -> dict:
    """Загрузить экспорт чата и получить все метрики одним JSON."""
    data, filename = await _read_upload(request, lang)
    try:
        ...
        raise HTTPException(422, exc.text(lang)) from exc
